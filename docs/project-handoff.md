...
2026-09-10 purple theme update: Added compact src/theme.ts using the supplied 16-step BrandVariants palette, createLightTheme/createDarkTheme, and dark colorBrandForeground1/2 overrides at steps 110/120. Inspected both supplied token exports; their brand background/foreground values agree with the requested palette. App theme toggle now supplies these themes to FluentProvider. Existing CSS and Fluent components inherit tokens; Stack and three sample rows preserved. Build/lint passed. Republished same app and verified light and dark purple rendering in hosted runtime 20260910t203357zdd63fbf3cb. README explains copying/customizing the theme. PR #3 is merged. Next: use theme.ts as the single source when changing brand colors.
